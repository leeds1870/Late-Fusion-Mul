...
def get_args() -> argparse.Namespace:
    ...
    engram_group.add_argument('--engram_hash_size', type=int, default=262144, #131072
    ...
    # ===== 新增：Late Fusion参数组 =====
    late_fusion_group = parser.add_argument_group(
        'Late Fusion架构参数（终极ELF优化）' )
    
    late_fusion_group.add_argument('--enable_late_fusion',action='store_true',
        default=True,help='启用Late Fusion架构（推荐，融合所有ELF优势）')
    
    late_fusion_group.add_argument('--decoder_split_layer',type=int,default=6,
        help='解码器分割点（前N层冻结，默认6）')
    
    late_fusion_group.add_argument('--enable_continuous_lora',action='store_true',
        default=True, help='启用Continuous-Space LoRA（作用于SONAR空间）')
    
    late_fusion_group.add_argument('--enable_soft_decoder',action='store_true',
        default=True,help='启用ELF Soft Decoder（温度可学习的最后离散化）')
    
    late_fusion_group.add_argument('--soft_decoder_temperature',type=float,default=1.0,
        help='Soft Decoder初始温度（默认1.0）')
    
    late_fusion_group.add_argument( '--enable_progressive_unfreezing', action='store_true',default=False,
        help='【实验性】启用渐进解冻策略' )
...
def engram_enhanced_generate(
    ...
    """
    【核心修复】Engram 增强自回归解码器（适用于标准 LoRA 模型）
    """
    model.eval()
    encoder = model.get_encoder()
    ...
    if sonar_autoencoder is not None:
        try:
            ...
            raw_emb = _encode_with_base_model(model, src_inp, device)
    ...
    for _ in range(max_length):
        ...
        hidden = dec_out.hidden_states[-1]
        B, T, D = hidden.shape
...
def translate_auto(
    ...
    lang_map: Dict[str, str] = None,
    engram_table=None, engram_gating=None, sonar_autoencoder=None 
) -> str:
    if lang_map is None:
        lang_map = LANG_MAP
    ...
    # 【核心分支】：Late Fusion 模型自带 generate，标准模型走 Engram 闭环
    if hasattr(model, 'continuous_loras'): 
        # LateFusionArchitecture 内部已处理（或需后续为其单独补充Engram注入）
        out_ids = model.generate(
            input_ids=inputs["input_ids"],
            attention_mask=inputs["attention_mask"],
            max_length=512
        )
    elif engram_table is not None and engram_gating is not None:
        out_ids = engram_enhanced_generate(
            model=model, tokenizer=tokenizer,
            engram_table=engram_table, engram_gating=engram_gating,
            sonar_autoencoder=sonar_autoencoder,
            input_ids=inputs["input_ids"], attention_mask=inputs["attention_mask"],
            tgt_lang_id=tgt_id, max_length=512, device=device
        )
    else:
        out_ids = model.generate(
            **inputs, forced_bos_token_id=tgt_id, max_length=512,
            num_beams=3, repetition_penalty=1.5, no_repeat_ngram_size=3, length_penalty=1.0,
        )
...
# ============================================================
# ============================================================
# 方案A：Flow-guided LoRA Layer【修改】FlowLoRALayer：添加flow_loss存储
# ============================================================
...
class FlowLoRALayer(nn.Module):
    ...
    【修改点】：
      添加_last_flow_loss属性，供EngramJointTrainer收集
    """
    ...
    def flow_forward(
        ...
        delta_h_target = self.lora_B(self.lora_A(x_dropped))
        
        B, T, D = delta_h_target.shape
        t = torch.rand(B, 1, 1, device=x.device)
        
        noise = torch.randn_like(delta_h_target)
        delta_h_t = (1 - t) * noise + t * delta_h_target
        
        delta_h_pred = delta_h_target
    ...
    def forward(
        ...
        """
        【修改】统一前向接口（存储flow_loss）
        
        修改点：
          将flow_loss存储到self._last_flow_loss
          供EngramJointTrainer._collect_flora_loss()收集
        """
...
def replace_lora_with_flora(
    ...
    使用场景：
      在get_peft_model()后调用，无缝升级为FLoRA
    
    Args:
        peft_model: PEFT包装后的模型
        flow_steps: Flow内部迭代次数
        flow_alpha_schedule: MSE权重调度策略
    ...
    for name, module in peft_model.named_modules():
        ...
        if hasattr(module, 'lora_A') and hasattr(module, 'lora_B'):
            ...
            alpha = getattr(module, 'scaling', 1.0) * rank
            dropout_p = getattr(module, 'lora_dropout', nn.Identity()).p \
                        if hasattr(getattr(module, 'lora_dropout', None), 'p') else 0.1
            ...
            # 复制权重（保持预训练初始化）
            flora_layer.lora_A.weight.data.copy_(module.lora_A.weight.data)
            flora_layer.lora_B.weight.data.copy_(module.lora_B.weight.data)
    ...
    if replaced_count > 0:
        ...
        logging.getLogger(__name__).info(
            f"  ✓ 已将{replaced_count}个标准LoRA层替换为FlowLoRA"
        )
...
# ============================================================
# 【新增】连续空间LoRA + Late Fusion架构
# ============================================================
# 【修改】ContinuousSpaceLoRA：添加flow_loss存储

class ContinuousSpaceConceptInjector(nn.Module):
    """
    【修复1】概念交叉注意力注入器（替代原 ContinuousSpaceLoRA）
    
    设计思想：
      原 CS-LoRA 将句子级 SONAR 向量直接相加到 Token 级 Hidden 上，
      导致每个 Token 被迫接受相同的全局静态偏置，破坏了自回归因果律。
      本模块采用 Cross-Attention 路由机制：
      - Query: Decoder 当前 Token 的隐藏态 [B, T_tgt, dim]
      - Key/Value: 源端 SONAR 概念空间 [B, L_src, dim]
      实现 Token 级别的动态概念提取，完美契合“概念空间解码”哲学。
      
    参数量：
      4 × (dim × rank) ≈ 4 × 1024 × 16 = 65k (极度轻量)
    """
    def __init__(self, dim: int = 1024, rank: int = 16, dropout: float = 0.1):
        super().__init__()
        self.rank = rank
        # 低秩投影以控制参数量（延续 LoRA 的参数高效思想）
        ...
        # 兼容原有的 flow_loss 收集接口（置为0，因注意力机制自带平滑性）
        self._last_flow_loss = None

    def forward(self, decoder_hidden: torch.Tensor, sonar_emb: torch.Tensor, return_flow_loss: bool = False):
        """
        Args:
            decoder_hidden: [B, T_tgt, dim] 目标端当前隐藏态
            sonar_emb: [B, L_src, dim] 源端概念空间（全局Context）
        """
        # 1. 投影到概念空间
        q = self.W_q(decoder_hidden)  # [B, T_tgt, rank]
        k = self.W_k(sonar_emb)       # [B, L_src, rank]
        v = self.W_v(sonar_emb)       # [B, L_src, rank]
        
        # 2. 注意力路由：动态决定当前 Token 需要多少 SONAR 概念
        attn_weights = (q @ k.transpose(-2, -1)) * self.scale # [B, T_tgt, L_src]
        attn_weights = F.softmax(attn_weights, dim=-1)
        attn_weights = self.dropout(attn_weights)
        
        # 3. 概念提取与残差投影
        concept_delta = attn_weights @ v # [B, T_tgt, rank]
        out = self.out_proj(concept_delta) # [B, T_tgt, dim]
        
        # 兼容 LateFusionTrainer 的收集接口
        self._last_flow_loss = torch.tensor(0.0, device=decoder_hidden.device)
        
        if return_flow_loss:
            return out, self._last_flow_loss
        return out
    
# ============================================================
# 核心创新2：ELF Soft Decoder（延迟离散化的最后一跳）
# ============================================================
class ELFSoftDecoder(nn.Module):
    """
    【修复2】ELF风格的软解码器（防Temperature坍塌版）
    
    核心修复：
      引入熵正则化（Entropy Penalty），惩罚过度尖锐的概率分布。
      这能防止优化器将 Temperature 推向 clamp 的下界，
      确保连续 Hidden 到离散 Token 的过渡真正具备“软化”效果，
      从而在低资源术语翻译中保留更多的长尾候选概率。
    """
    def __init__(
        self,
        hidden_dim: int = 1024,
    ...
    def forward(
        ...
        return_soft_loss: bool = False,
    ) -> Dict[str, torch.Tensor]:
        logits = self.projection(hidden_states)
        
        # 【修复】限制 Temperature 范围，防止梯度爆炸或坍塌
        temp = self.temperature.clamp(min=0.5, max=2.0) 
        soft_logits = logits / temp
        
        result = {'logits': logits, 'soft_logits': soft_logits}
        
        if return_soft_loss and labels is not None:
            # 1. 标准硬损失
            ...
            soft_loss = F.cross_entropy(
                soft_logits.view(-1, self.vocab_size), 
                labels.view(-1), 
                ignore_index=-100
            )
            
            # 3. 【核心修复】熵正则化：惩罚过度尖锐的分布，保护 Temperature
            probs = F.softmax(soft_logits, dim=-1)
            entropy = -(probs * torch.log(probs + 1e-9)).sum(dim=-1).mean()
            entropy_penalty = -0.05 * entropy  # 鼓励一定的平滑度
            
            # 综合损失
            total_loss = 0.7 * hard_loss + 0.3 * soft_loss + entropy_penalty
            
            result['soft_loss'] = total_loss
            result['hard_loss'] = hard_loss
            result['temperature'] = temp.item()
    ...
    def generate_step(
        ...
        if top_k > 0:
            soft_logits_2d = soft_logits.squeeze(1)
            top_k_logits, top_k_indices = soft_logits_2d.topk(top_k, dim=-1)
            mask = torch.full_like(soft_logits_2d, float('-inf'))
        ...
        if do_sample:
            probs = F.softmax(soft_logits_2d, dim=-1)
            next_token = torch.multinomial(probs, num_samples=1)
        else:
            next_token = soft_logits_2d.argmax(dim=-1, keepdim=True)
            
        return next_token
    
# ============================================================
# 核心创新3：Late Fusion Architecture
# ============================================================

class LateFusionArchitecture(nn.Module):
    """
    【终极方案】融合所有ELF优势的Late Fusion架构
    ...
    流水线：
      ...
      SONAR空间（连续优化，无LoRA）
        ↓
      NLLB Decoder前半（Layer 1-6，彻底冻结）
        ↓
      NLLB Decoder后半（Layer 7-12，概念交叉注意力注入）★ELF核心
        ↓
      ELF Soft Decoder（最后离散化 + 熵正则化）★
        ↓
      tokens
    """
    def __init__(
        self,
        base_model,             # 预训练NLLB模型
        sonar_autoencoder,      # 预训练SONAR
        engram_gating,          # Engram门控网络
        decoder_split_layer: int = 6,  # 解码器分割点（前6层冻结）
        lora_rank: int = 16,    # 后半解码器的概念注入秩
        enable_soft_decoder: bool = True,  # 是否使用ELF Soft Decoder
        device: str = "cuda",
    ):
        super().__init__()
        ...
        self.base_model = base_model
        self.sonar = sonar_autoencoder
        self.engram_gating = engram_gating
        self.decoder_split_layer = decoder_split_layer
        self.device = device
        
        # ===== 【核心修复1】彻底冻结整个 base_model =====
        # 原逻辑仅冻结Encoder和前6层，导致后6层和lm_head默认开启梯度，
        # 不仅显存爆炸，还会导致Trainer收集到数百兆的非预期base_model参数，
        # 违背了 Late Fusion “冻结基座，仅训练附加组件” 的设计初衷。
        for param in self.base_model.parameters():
            param.requires_grad = False
            
        # ===== 【核心修复2】后半程：概念交叉注意力注入器 =====
        # 替代原有的 ContinuousSpaceLoRA（直接相加会导致全局静态偏置灾难）。
        # 采用 Cross-Attention 路由机制：
        # Query = Decoder当前Token Hidden
        # Key/Value = 源端全局 SONAR 概念空间
        # 让每个Token动态“检索”所需的连续概念，完美契合自回归因果律。
        decoder_layers = self.base_model.model.decoder.layers
        self.concept_injectors = nn.ModuleList()
        
        for i in range(decoder_split_layer, len(decoder_layers)):
            injector = ContinuousSpaceConceptInjector(
                dim=1024,
                rank=lora_rank,
                dropout=0.1,
            )
            self.concept_injectors.append(injector)
            
        # ===== ELF Soft Decoder (带熵正则化的软离散化) =====
        if enable_soft_decoder:
            self.soft_decoder = ELFSoftDecoder(
                hidden_dim=1024,
            ...
            # 复制预训练 lm_head 权重以加速收敛，避免冷启动
            if hasattr(base_model, 'lm_head'):
                self.soft_decoder.projection.weight.data.copy_(
                    base_model.lm_head.weight.data
                )
        else:
            self.soft_decoder = None
            
        # ===== SONAR嵌入缓存（避免自回归推理时重复计算）=====
        self.sonar_cache = {}
    ...
    def _get_sonar_embedding(
        ...
        Returns:
            sonar_emb: [B, L, 1024] 每个token对应的SONAR嵌入
        """
        # 缓存检查
        cache_key = hash(input_ids.cpu().numpy().tobytes())
        if cache_key in self.sonar_cache:
            return self.sonar_cache[cache_key].to(self.device)
        
        # 计算SONAR嵌入
        with torch.no_grad():
            # 使用SONAR编码器（sequence-level）
            sonar_out = self.sonar(
                input_ids, 
                attention_mask,
                lang_code='eng_Latn'  # 默认，实际应从tokenizer获取
            )
            sonar_emb = sonar_out['normalized']  # [B, 1024]
            
            # 扩展到sequence维度（每个token共享句子级SONAR嵌入）
            # 注意：这是简化方案，理想情况应该有token-level SONAR
            L = input_ids.shape[1]
            sonar_emb = sonar_emb.unsqueeze(1).expand(-1, L, -1)  # [B, L, 1024]
        
        # 缓存
        self.sonar_cache[cache_key] = sonar_emb.cpu()
        
        return sonar_emb
    
    def forward(self, input_ids, attention_mask, decoder_input_ids=None, labels=None, **kwargs):
        # Step 1 & 2 & 3 保持不变 (Encoder, SONAR, Decoder前6层)
        with torch.no_grad():
        ...
        sonar_emb = self._get_sonar_embedding(input_ids, attention_mask)
        
        with torch.no_grad():
            decoder_layers = self.base_model.model.decoder.layers
            if decoder_input_ids is None:
                decoder_input_ids = self._shift_right(labels)
            hidden_states = self.base_model.model.decoder.embed_tokens(decoder_input_ids)
            for i in range(self.decoder_split_layer):
                layer_outputs = decoder_layers[i](
                    hidden_states,
        ...
        # ===== Step 4: Decoder后6层（概念注入）=====
        for i, injector in enumerate(self.concept_injectors):
            layer_idx = self.decoder_split_layer + i
            layer_outputs = decoder_layers[layer_idx](
                hidden_states,
                encoder_hidden_states=encoder_hidden,
                encoder_attention_mask=attention_mask,
            )
            base_hidden = layer_outputs[0]
            
            # 【修复】动态路由注入：当前步Hidden 与 全局SONAR 交互
            concept_delta = injector(base_hidden, sonar_emb)
            hidden_states = base_hidden + concept_delta
            
        # Step 5: ELF Soft Decoder (保持不变)
        if self.soft_decoder is not None:
            decoder_output = self.soft_decoder(
                hidden_states, labels=labels, return_soft_loss=(labels is not None)
            )
            logits = decoder_output['soft_logits']
            loss = decoder_output.get('soft_loss', None)
        else:
    ...
    def generate(self, input_ids, attention_mask, max_length=128, **kwargs):
        ...
        decoder_input_ids = torch.full(
            (batch_size, 1), self.base_model.config.decoder_start_token_id,
        ...
        for step in range(max_length):
            ...
            for i, injector in enumerate(self.concept_injectors):
                ...
                # 删除了原代码中致命的 sonar_emb[:, :current_seq_len, :] 裁剪！
                # 让 Cross-Attention 自动决定当前目标 Token 关注源端的哪个概念
                concept_delta = injector(base_hidden, sonar_emb)
                hidden_states = base_hidden + concept_delta
                
            if self.soft_decoder is not None:
                last_hidden = hidden_states[:, -1:, :]
                next_token = self.soft_decoder.generate_step(last_hidden, do_sample=False, top_k=50)
            else:
                last_hidden = hidden_states[:, -1, :]
    ...
    def _shift_right(self, labels: torch.Tensor) -> torch.Tensor:
        """右移labels构建decoder_input_ids"""
        shifted = labels.new_zeros(labels.shape)
        shifted[:, 1:] = labels[:, :-1].clone()
        shifted[:, 0] = self.base_model.config.decoder_start_token_id
        return shifted
    

# ============================================================
# 辅助函数：构建Late Fusion模型
# ============================================================
...
def build_late_fusion_model(
    ...
    """
    构建Late Fusion架构（替代标准LoRA注入）
    
    使用场景：
      在alignment_training()中，替代get_peft_model()
    ...
    Returns:
        LateFusionArchitecture实例
    """
    logger = logging.getLogger(__name__)
    
    model = LateFusionArchitecture(
        base_model=base_model,
        sonar_autoencoder=sonar_autoencoder,
        engram_gating=engram_gating,
        decoder_split_layer=args.decoder_split_layer,  # 默认6
        lora_rank=args.flora_rank,  # 默认16
        enable_soft_decoder=args.enable_soft_decoder,  # 默认True
        device=args.device,
    )
    ...
    logger.info(
        f"  [Late Fusion] 模型构建完成\n"
        f"    冻结参数: {frozen_params:,} ({frozen_params/1e6:.1f}M)\n"
        f"    可训练参数: {trainable_params:,} ({trainable_params/1e6:.1f}M)\n"
        f"    总参数: {(frozen_params+trainable_params)/1e6:.1f}M\n"
        f"    可训练占比: {trainable_params/(frozen_params+trainable_params):.2%}"
    )
...
class AdaptiveLossBalancer:
    ...
    """
    def __init__(self, loss_names: List[str], momentum: float = 0.95):
...
class EngramJointTrainer(CGLoRATrainer):
    """
    Engram + SONAR + LoRA 三重联合训练器
    
    核心改进：
      1. 双优化器：LoRA参数 + Engram门控网络独立优化
      2. 三重损失：L_CE + λ_sonar·L_SONAR + λ_engram·L_Engram
      3. 阶段性训练：Warmup → Finetune → (可选)Align
      4. 动态权重调度：根据训练阶段自动调整损失权重
    
    损失权重调度（3阶段）：
      阶段B（Warmup, Epoch 1~3）：
        λ_sonar  = 0.05（低，稳定初期）
        λ_engram = 0.15（高，快速学习门控）
      
      阶段C（Finetune, Epoch 4~12）：
        λ_sonar  = 0.15（正常）
        λ_engram = 0.10（平衡）
      
      阶段D（Align, Epoch 13~15，可选）：
        λ_sonar  = 0.25（高精度）
        λ_engram = 0.05（固化记忆）
    
    Engram + SONAR + LoRA 三重联合训练器（双优化器包装版）
    
    修复说明：
      1. create_optimizer() 返回 DualOptimizerWrapper 实例
         并显式赋值给 self.optimizer（兼容 Trainer 接口）
      2. training_step() 移除手动的 optimizer.step() 调用
         让 Trainer._inner_training_loop 自动处理
      3. tokenizer 参数改为从 **kwargs 获取（兼容未来版本）
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        FutureWarning 修复说明：
          从 kwargs 中提取 tokenizer 参数
          转换为 processing_class 后传给父类
          避免 Trainer.__init__ 触发弃用警告
        
        兼容性：
          同时支持旧版（tokenizer=）和新版（processing_class=）
          优先使用 processing_class（若两者都传，processing_class 优先）
        """
       # ── 核心修复：tokenizer → processing_class 转换 ──
        tokenizer_obj = kwargs.pop('tokenizer', None)
        if 'processing_class' not in kwargs and tokenizer_obj is not None:
        ...
        # 【修复】恢复 lambda 属性（作为课程学习基准和 Early Return 开关）
    ...
    def create_optimizer(self) -> DualOptimizerWrapper:
        ...
        返回 DualOptimizerWrapper 实例，兼容 Trainer 的所有优化器调用
        同时显式赋值给 self.optimizer（必须！）
        ...
        # 【核心修复】包装并显式赋值给 self.optimizer
        dual_optimizer = DualOptimizerWrapper(
            self.optimizer_lora,
            self.optimizer_engram
        )
        self.optimizer = dual_optimizer  # ← 必须！让 Trainer 能访问
    ...
    def create_scheduler(
        ...
        注意：optimizer 参数是 Trainer 传入的 self.optimizer（即 DualOptimizerWrapper）
        我们直接使用内部的 optimizer_lora 和 optimizer_engram
        ...
        self.lr_scheduler = dual_scheduler  # Trainer 期望的属性名
    ...
    # ── 其他方法保持不变（_pool_hidden / compute_loss / _compute_* 等）──
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if self.enable_flora and model.training:
            ...
            if hasattr(self, 'state') and self.state.max_steps > 0:
                progress = self.state.global_step / self.state.max_steps
                alpha_mse = 0.2 + (self.flow_mse_weight_max - 0.2) * (1 - math.cos(math.pi * progress)) / 2
            else:
                alpha_mse = self.flow_mse_weight_max * 0.5
        ...
        # 结合课程学习(lambda)与动态平衡(weights)
        total_loss = (
            weights['ce'] * ce_loss +
            weights['sonar'] * self.lambda_sonar * sonar_loss +
            weights['engram'] * self.lambda_engram * engram_loss +
            weights['mse'] * alpha_mse * mse_loss
        )
    ...
    def _run_engram_gating(self, outputs, inputs, src_anchor, device):
        """Engram门控执行（代码与之前一致）"""
        logger   = logging.getLogger(__name__)
        gate_info = {'mean_gate': 0.0, 'active_ratio': 0.0}
        avg_conf  = 0.0
    ...
    def training_step(
        ...
        """
        【核心修复】单步训练（移除手动optimizer.step()）
        
        修复说明：
          仅执行前向+反向，不再手动调用 optimizer.step()
          让 Trainer._inner_training_loop 自动调用 self.optimizer.step()
          （实际调用的是 DualOptimizerWrapper.step()，同步更新两个优化器）
        ...
        # 4. 返回loss（Trainer会自动调用optimizer.step()）
        return loss.detach()
...
# ============================================================
# 【修改】LateFusionTrainer：专为Late Fusion设计的训练器
# ============================================================

class LateFusionTrainer(EngramJointTrainer):
    """
    Late Fusion专用训练器
    """
    def __init__(
        self,
        late_fusion_model: "LateFusionArchitecture",
        enable_progressive_unfreezing: bool = False,
        **kwargs
    ):
        # 【修复2】无条件从 kwargs 中 pop 出所有可能冲突的参数
        # 防止后续 super().__init__ 报 "multiple values for keyword argument" 错误
        engram_table = kwargs.pop('engram_table', None)
        engram_gating_from_kwargs = kwargs.pop('engram_gating', None)
        sonar_autoencoder_from_kwargs = kwargs.pop('sonar_autoencoder', None)
        cfg_args = kwargs.pop('cfg_args')
        
        # 优先从 late_fusion_model 获取组件，若无则使用 kwargs 传入的
        engram_gating = getattr(late_fusion_model, 'engram_gating', engram_gating_from_kwargs)
        sonar_autoencoder = getattr(late_fusion_model, 'sonar', sonar_autoencoder_from_kwargs)
        
        # 将 late_fusion_model 作为 base model 传递
        kwargs['model'] = late_fusion_model
        
        # 调用父类（EngramJointTrainer）
        super().__init__(
            engram_table=engram_table,
            engram_gating=engram_gating,
            sonar_autoencoder=sonar_autoencoder,
        ...
        self.enable_progressive_unfreezing = enable_progressive_unfreezing
        
        # ===== 【核心修复】覆盖父类的 lora_params 收集逻辑 =====
        # 父类 EngramJointTrainer 使用 'lora' in n.lower() 筛选，
        # 但 LateFusion 的 ConceptInjector 和 SoftDecoder 参数名不含 'lora'，
        # 导致 self.lora_params 为空，引发 "optimizer got an empty parameter list"。
        
        # 1. 获取 engram 参数的 ID 集合（避免同一个参数被两个优化器重复更新）
        engram_param_ids = {id(p) for p in self.engram_params}
        
        # 2. 收集 LateFusion 模型中所有需要梯度且不属于 engram 的参数
        self.lora_params = [
            p for p in self.late_fusion_model.parameters() 
            if p.requires_grad and id(p) not in engram_param_ids
        ]
        
        if not self.lora_params:
            raise ValueError(
                "LateFusion 模型中没有找到可训练参数！\n"
                "请检查 ConceptInjector 和 SoftDecoder 的 requires_grad 状态。"
            )
            
        # 统计并打印可训练参数（用于调试确认）
        trainable_count = sum(p.numel() for p in self.lora_params)
        import logging
        logging.getLogger(__name__).info(
            f"  [LateFusionTrainer] 成功收集 {len(self.lora_params)} 个参数张量 "
            f"({trainable_count:,} 参数) 注入 LoRA 优化器"
        )
    ...
    def _collect_cs_lora_loss(self) -> torch.Tensor:
        """
        收集所有 Concept Injector 的辅助损失
        (注：Cross-Attention 自带平滑性，此处返回0.0以兼容框架接口)
        """
        cs_lora_losses = []
        # 【适配】遍历新的 concept_injectors
        for injector in self.late_fusion_model.concept_injectors:
            if hasattr(injector, '_last_flow_loss') and injector._last_flow_loss is not None:
                cs_lora_losses.append(injector._last_flow_loss)
                
        if cs_lora_losses:
            return torch.stack(cs_lora_losses).mean()
    ...
    def _apply_progressive_unfreezing(self, epoch: int):
        ...
        if epoch in self.unfreeze_schedule:
            target_layer = self.unfreeze_schedule[epoch]
            
            logger = logging.getLogger(__name__)
            logger.info(
                f"  [渐进解冻] Epoch {epoch}: "
                f"解冻Decoder第{target_layer}-{len(self.late_fusion_model.continuous_loras)+self.late_fusion_model.decoder_split_layer-1}层"
            )
            
            # 解冻对应层
            decoder_layers = self.late_fusion_model.base_model.model.decoder.layers
            for i in range(target_layer, self.late_fusion_model.decoder_split_layer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        soft_loss = outputs_dict.get('loss', torch.tensor(0.0, device=device))
        
        sonar_loss = torch.tensor(0.0, device=device)
        if self.lambda_sonar > 0:
            encoder_hidden = outputs_dict.get('encoder_last_hidden_state')
            if encoder_hidden is not None:
                src_anchor = self._compute_src_anchor(model, inputs, device)
                enc_pooled = self._pool_hidden(encoder_hidden, inputs['attention_mask'])
                sonar_loss = (1 - F.cosine_similarity(enc_pooled, src_anchor, dim=-1)).mean()
        
        engram_loss = torch.tensor(0.0, device=device)
        
        cs_lora_loss = torch.tensor(0.0, device=device)
        alpha_cs = 0.0
        if self.enable_flora and model.training:
            cs_lora_loss = self._collect_cs_lora_loss()
            if hasattr(self, 'state') and self.state.max_steps > 0:
                progress = self.state.global_step / self.state.max_steps
                alpha_cs = 0.2 + (self.flow_mse_weight_max - 0.2) * (1 - math.cos(math.pi * progress)) / 2
            else:
                alpha_cs = self.flow_mse_weight_max * 0.5

        # ===== 【核心修改】应用 AdaptiveLossBalancer =====
        current_losses = {
            'ce': soft_loss.item(),
            'sonar': sonar_loss.item(),
            'engram': engram_loss.item(),
            'mse': cs_lora_loss.item()
        }
        weights = self.loss_balancer.get_weights(current_losses)
        
        total_loss = (
            weights['ce'] * soft_loss +
            weights['sonar'] * self.lambda_sonar * sonar_loss +
            weights['engram'] * self.lambda_engram * engram_loss +
            weights['mse'] * alpha_cs * cs_lora_loss
        )
        ...
        if return_outputs:
            ...
            outputs = Seq2SeqLMOutput(
                ...
                encoder_last_hidden_state=outputs_dict.get('encoder_last_hidden_state'),
            )
    ...
    def on_epoch_begin(self, args, state, control, **kwargs):
        ...
        self._apply_progressive_unfreezing(current_epoch)
...
# 【修改】alignment_training：集成Late Fusion
# 【修复】alignment_training：统一使用EngramJointTrainer
# ============================================================
...
def alignment_training(
    ...
    """
    【修复版】第二步：跨语言对齐训练（Late Fusion优先）
    
    修复点：
      1. 所有FlowEngramJointTrainer调用改为EngramJointTrainer
      2. 确保Late Fusion路径正确
    """
    logger = logging.getLogger(__name__)
    ...
    # ===== SONAR预训练（前面代码保持不变）=====
    sonar_autoencoder = None
    if args.use_sonar or args.enable_late_fusion:
        ...
        if sonar_autoencoder is None:
            logger.warning("[SONAR] 预训练失败")
            if args.enable_late_fusion:
                raise ValueError(
                    "Late Fusion依赖SONAR，但SONAR预训练失败！"
                )
    ...
    if args.enable_engram or args.enable_late_fusion:
        ...
        if args.enable_flow_engram:
            engram_gating = FlowEngramGating(
                ...
                accumulate_K=args.flow_engram_K,
                enable_flow_aggregate=True,
            )
            logger.info(f"  [Engram] FlowEngramGating (K={args.flow_engram_K})")
        else:
            engram_gating = SOARGuidedEngramGating(
                nllb_dim=1024,
                sonar_dim=1024,
    ...
    # ===== 跨语言对提取（保持不变）=====
    use_sonar_effective = (sonar_autoencoder is not None)
    ...
    if args.enable_late_fusion:
        # ===== Late Fusion路径 =====
        logger.info("  [模型] 使用Late Fusion架构")
        ...
        logger.info("  ✓ Late Fusion模型构建完成")
    
    else:
        # ===== 标准LoRA路径 =====
        logger.info("  [模型] 使用标准LoRA")
    ...
    if not cross_pairs:
        ...
        if args.enable_late_fusion:
            torch.save({
                'continuous_loras': model.continuous_loras.state_dict(),
                'soft_decoder': model.soft_decoder.state_dict() if model.soft_decoder else None,
                'config': {
                    'decoder_split_layer': model.decoder_split_layer,
                    'lora_rank': args.flora_rank,
                }
            }, os.path.join(save_dir, "late_fusion_weights.pt"))
        else:
    ...
    # ===== 预计算初始NLLB编码对（Late Fusion中可能跳过）=====
    initial_encoded = []
    if not args.enable_late_fusion:
    ...
    logger.info(
        f"  [对齐训练] "
        f"架构={'Late Fusion' if args.enable_late_fusion else 'Standard LoRA'} | "
        f"SONAR={'ON' if use_sonar_effective else 'OFF'} | "
        f"Engram={'ON' if args.enable_engram else 'OFF'} | "
        f"对数={len(cross_pairs)} | "
    ...
    # ===== 【修复】选择训练器：统一使用EngramJointTrainer =====
    if args.enable_late_fusion:
        # ===== Late Fusion专用训练器 =====
        trainer = LateFusionTrainer(
            late_fusion_model=model,
            enable_progressive_unfreezing=args.enable_progressive_unfreezing,
            
            # 从kwargs传递engram组件（避免重复）
            engram_table=engram_table,
            engram_gating=engram_gating,  # Late Fusion内部已有，但仍传递以兼容
            sonar_autoencoder=sonar_autoencoder,
            cfg_args=args,
            
            # FLoRA参数
            enable_flora=args.enable_continuous_lora,
        ...
        logger.info("  ✓ 使用LateFusionTrainer（终极优化）")
    
    elif args.enable_engram and engram_table and engram_gating:
        # ===== 【修复】使用EngramJointTrainer（不是FlowEngramJointTrainer）=====
        trainer = EngramJointTrainer(  # ← 修复：改为EngramJointTrainer
            engram_table=engram_table,
            engram_gating=engram_gating,
            sonar_autoencoder=sonar_autoencoder,
            cfg_args=args,
        ...
        logger.info("  ✓ 使用EngramJointTrainer（集成FLoRA+FlowEngram）")
    ...
    if args.enable_late_fusion:
        torch.save({
            'continuous_loras': model.continuous_loras.state_dict(),
            'soft_decoder': model.soft_decoder.state_dict() if model.soft_decoder else None,
            'engram_gating': engram_gating.state_dict() if engram_gating else None,
            'config': {
                'decoder_split_layer': model.decoder_split_layer,
                'lora_rank': args.flora_rank,
                'enable_soft_decoder': args.enable_soft_decoder,
                'sonar_dim': 1024,
            }
        }, os.path.join(save_dir, "late_fusion_weights.pt"))
        
        logger.info(f"  [Late Fusion] 权重已保存: {save_dir}/late_fusion_weights.pt")
    else:
        model.save_pretrained(save_dir)
    ...
    if args.enable_engram and engram_gating:
        ...
        torch.save({
            ...
            'config': {
                ...
                'gate_threshold': args.engram_gate_threshold,
                'accumulate_K': args.flow_engram_K,
                'enable_flow_aggregate': args.enable_flow_engram,
            }
        }, engram_save_path)
...
def main():
    ...
    if args.do_predict:
        ...
        # 【修复2】从本地缓存加载 Engram 嵌入表 (解决 engram_table 未定义)
        engram_table_loaded = None
        if args.enable_engram:
            try:
                engram_table_loaded = TerminologyEngramTable(
                    nllb_dim=1024,
                    max_ngram=args.engram_max_ngram,
                    hash_table_size=args.engram_hash_size,
                    device=args.device,
                    cache_dir=r'F:\hyberT\MultiSonar\data\Engram_table' # 确保与训练时路径一致
                )
                if engram_table_loaded.load_from_cache():
                    logger.info("  ✓ Engram嵌入表已从本地缓存加载 (O(1) 耗时)")
                else:
                    logger.warning("  ⚠️ Engram嵌入表缓存未找到，推理时Engram检索将降级")
                    engram_table_loaded = None
            except Exception as e:
                logger.warning(f"  ⚠️ Engram嵌入表加载失败: {e}")
                engram_table_loaded = None

        # 【修复3】SONAR 安全降级 (解决 sonar_autoencoder 未定义)
        # 理论说明：推理时 _compute_src_anchor 若没有完整的 autoencoder 绑定 base_model，
        # 会安全降级为零向量 (torch.zeros)。这不会影响核心的 Engram N-gram 检索，
        # 仅减弱全局语义引导，是推理阶段最具性价比的防御性策略。
        sonar_autoencoder_loaded = None

        # 执行预测（传入安全加载/降级后的组件）
        finetuned_result = predict(
            args, model, tokenizer, args.test_text,
            engram_table=engram_table_loaded,
            engram_gating=engram_gating_loaded,
            sonar_autoencoder=sonar_autoencoder_loaded
        )
